...
class Trainer:
    def __init__(self, args, config, device):
        ...
        # networks
        self.encoder = get_encoder_from_config(
            self.config["networks"]["encoder"]).to(device)
        self.classifier = get_classifier_from_config(
            self.config["networks"]["classifier"]).to(device)
    ...
    def _do_epoch(self):
        ...
        with torch.no_grad():
            ...
            if self.results['val'][self.current_epoch] >= self.best_val_acc:
                ...
                self.logger.save_best_model(self.encoder, self.classifier,
                                            self.best_val_acc)

    def do_eval(self, loader):
        correct = 0
        for it, (batch, domain) in enumerate(loader):
            data, labels, domains = batch[0].to(self.device), batch[1].to(
                self.device), domain.to(self.device)
    ...
    def do_training(self):
        ...
        self.logger.save_best_acc(val_res, test_res, self.best_val_acc,
